...
The __Gaussian process__ attempts to describe a directional dependency between a covariate variable $$x$$ and the corresponding observable output $$y$$. For doing that it uses the conditional distribution $$p(y\vert x)$$ which describes the dependency of an observable $$y$$ on acorresponding input $$x \in X$$. 
A random function $$f:X\to \mathbf{R}$$ is a Gaussian Process with mean $$m(x)$$ and co-variance function $$k(x,x*)$$ if $$f_{X}=(f(x_1),...,f(x_n)) \sim \mathcal{N}(\mu_X,K_{XX})$$ (where $$\mu$$ is often considered as zero). This method aim to study $$y=f(x)+\epsilon$$, where $$\epsilon \sim \mathcal{N}(0,\sigma^2)$$, in particular it wants to learn $$f$$ in order to make predictions on $$f(x*)$$. In this case $$y$$ is distributed as $$\mathcal{N}(0,K(x,x)+\sigma^2I)$$, because of the noise factor. To reach those results, it considers a training set $$\{(x_i,y_i), i=1,...,n\}$$ and a testing set $$\{x_i^\star, i=1,...,n\}$$, furthemore, it takes advantage of the likelihood function $$p(y\vert f, \theta)=\mathcal{N}(f, \sigma^2I)$$, which can be defined. The $$\theta$$ variable represents all the hyper-parameters that explain that are in the Kernel function. Particularly, this model aims to find the $$\theta$$ value that maximizes this loglikelihood. At this point, after it found the $$\theta$$ value, it can predict the $$f{\star}$$ distribution:
$$p(f^{\star}\vert, X, X^{\star}, \theta) = \mathcal{N}(K^{\star}(K+\sigma^2)^{-1}y, K^{\star\star}-K^{\star}(K−\sigma^2)^{-1}K^{\star})$$
