...
To infer the relationship between $x$ and $y$, it considers a training set $$\mathcal{D} = \{(x_i,y_i), i=1,...,n\}$$ and a testing set $$\{x_i^\star, i=1,...,n\}$$, , in particular it wants to learn $$f$$ in order to make predictions on $$f(x^\star)$$. Furthemore, it takes advantage of the marginal likelihood function to compute the posterior distribution of $y^*$ for $x^*$ conditioned on the information of the data $\mathcal{D}$. 
$$
