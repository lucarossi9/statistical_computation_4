...
The dataset used in our analysis is CHLA9.loom, containing various information about more than 5000 different cells. Notice that the initial format of our data is a loom format, designed to efficiently hold large omics datasets. Each column of the dataset described a different cell and it is named and identified by a unique string, part of this string corresponds to the DNA sequence of the cell. For each column of the dataset (for each cell) we have more than 60000 positive natural numbers associated to it. Each of these numbers represents a specific count of a quantity inside the cell. For each cell we have also an attribute called "TotalUMIs" representing the sum of the aforementioned counts. This number is often introduced in omic datasets since the most of the counts are zero (our dataset is sparse). Additionally, each row of our dataset is indexed by a list of keys. Among them, we decided to filter our initial dataset so that it contains only the cells whose genetype is  'protein_encoding' since it is generally easier to predict the phase of their cycle.
